...
The subset of {2,3,5,7,11} with the largest product that _does not exceed 100_ is {7,11}, with the product 77.

Given a set _s_ and a number _v_, define A(s,v) as the subset of s with the largest product that does not exceed v. Also, define p(n) as the set of the first n primes (thus p(5) is equal to {2,3,5,7,11}). Here are some examples of A(p(n), v):
